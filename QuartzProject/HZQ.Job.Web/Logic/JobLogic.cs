using HZQ.Job.Model;
using HZQ.Job.Web.Db;
using HZQ.Job.Web.Helper;
    ...
    internal class JobLogic
    {
        ...
        public JobResult Run(JobInfo jobInfo)
        {
            return Execute(() =>
            {
                ...
                return JsonConvert.DeserializeObject<JobResult>(PostJosn(path, jobInfo));
            });
        }
        ...
        /// <returns></returns>
        private bool ProcessResult(JobResult result, Func<int> func = null)
        {
            if (result.Code == 200)
            {
                if (func == null || func() > 0)
        ...
        /// <returns></returns>
        public JobResult Execute(Func<JobResult> func, [CallerMemberName] string method = null)
        {
            JobResult result = new JobResult { Code = 400 };
            try
            {
                result = func.Invoke();
            }
